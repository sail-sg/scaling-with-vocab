...
def forward_step(data_iterator, model):
    ...
    output_tensor = model(tokens, position_ids, attention_mask,
                          labels=labels, lookup_probabilities=lookup_probabilities)
...
def extra_args(parser):
    ...
    group.add_argument("--path_to_tokenid_probabilities", type=str, 
        help='path to the probabilities of tokenid.')
...
if __name__ == "__main__":
    ...
    global lookup_probabilities
    with open(args.path_to_tokenid_probabilities, 'rb') as f:
        tokenid_probabilities = pickle.load(f) # a dict
    max_key = max(tokenid_probabilities.keys())
    lookup_probabilities = torch.empty(max_key + 1)
