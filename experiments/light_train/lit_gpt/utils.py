...
def num_parameters(module: nn.Module, requires_grad=None, omit_embed=False) -> int:
    if omit_embed:
        parameters = (p for n,p in  module.named_parameters() if n!='transformer.wte.weight')
    else:
        parameters = module.parameters()
    return sum(p.numel() for p in parameters if requires_grad is None or p.requires_grad == requires_grad)
