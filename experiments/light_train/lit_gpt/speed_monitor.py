...
def estimate_flops(model: GPT) -> int:
    ...
    """
    n_trainable_params = num_parameters(model, requires_grad=True, omit_embed=False)
    trainable_flops = flops_per_param(model.config, n_trainable_params)
    # print('trainable_flops')
    # forward + backward + gradients (assumes no gradient accumulation)
    ops_per_step = 3 if model.training else 1
    n_frozen_params = num_parameters(model, requires_grad=False, omit_embed=False)
